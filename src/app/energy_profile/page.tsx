...
const T = {
  nl: {
    navName: 'Laura Dijcks', navRole: 'Lead Recruiter · Hire.nl', startCta: 'Start scan →',
    badge: 'Hire.nl · Kandidaatprofiel', heroTitle: 'Wie ben jij als je op je best bent?',
    ...
    benefits: [
      ...
      { icon: '💬', title: 'Een betere intake', body: 'Gebruik jouw profiel als gespreksonderwerp met Laura. Zo gaan we meteen de diepte in.' },
      { icon: '📧', title: 'Resultaat per e-mail', body: 'Je ontvangt jouw volledige profiel per e-mail, zodat je het kunt bewaren en delen.' },
    ],
    ctaTitle: 'Klaar?', ctaBtn: 'Start de scan →',
    footerLeft: 'Hire.nl · Laura Dijcks', footerRight: '© 2025 Hire.nl',
  },
  en: {
    navName: 'Laura Dijcks', navRole: 'Lead Recruiter · Hire.nl', startCta: 'Start scan →',
    badge: 'Hire.nl · Candidate Profile', heroTitle: 'Who are you at your best?',
    ...
    benefits: [
      ...
      { icon: '💬', title: 'A better intake conversation', body: 'Use your profile as a conversation starter with Laura. We can dive deep right away.' },
      { icon: '📧', title: 'Results by email', body: 'You receive your full profile by email, so you can keep it and share it.' },
    ],
    ctaTitle: 'Ready?', ctaBtn: 'Start the scan →',
    footerLeft: 'Hire.nl · Laura Dijcks', footerRight: '© 2025 Hire.nl',
  },
}

function EnergyProfileLandingInner() {
  const searchParams = useSearchParams()
  const router = useRouter()
  const rawLang = searchParams.get('lang') ?? 'nl'
  const lang = (['nl', 'en'].includes(rawLang) ? rawLang : 'nl') as Lang
  const switchLang = (l: Lang) => router.replace(`/energy_profile?lang=${l}`)
  const t = T[lang]
