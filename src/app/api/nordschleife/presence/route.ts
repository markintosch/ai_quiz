...
async function countActive(supabase: AnyClient): Promise<number> {
  ...
  if (error) {
    console.warn('[nordschleife/presence] count error:', error.message)
    return 0
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.warn('[nordschleife/presence] upsert error:', error.message)
    ...
    console.warn('[nordschleife/presence] error:', err)
    return NextResponse.json({ count: 0 }, { status: 200 })
  }
}
...
export async function GET() {
  ...
  try {
    ...
    console.warn('[nordschleife/presence] GET error:', err)
