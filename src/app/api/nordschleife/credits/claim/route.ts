...
import {
  ...
  decodeCreditCookie,
  encodeCreditCookie,
} from '@/lib/nordschleife/credits'
import { PAID_BUNDLE_ATTEMPTS, PAID_PRODUCT_SLUG } from '@/products/nordschleife/data'
...
export async function POST(req: NextRequest) {
  ...
  const rl = rateLimit(`ns_claim:${ip}`, 10, 10 * 60 * 1000)
  ...
  try {
    ...
    if (updErr) {
      console.error('[nordschleife/claim] update error:', updErr)
    ...
    console.error('[nordschleife/claim] error:', err)
