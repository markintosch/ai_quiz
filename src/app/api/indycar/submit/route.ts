...
import type { SectorResult } from '@/products/indycar/data'

export async function POST(req: NextRequest) {
  const ip = getClientIp(req.headers)
  const rl = rateLimit(`ic_submit:${ip}`, 15, 10 * 60 * 1000)
  ...
  try {
    ...
    if (insertError) {
      console.error('[indycar/submit] insert error:', insertError)
    ...
    console.error('[indycar/submit] error:', err)
