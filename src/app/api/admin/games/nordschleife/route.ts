...
export async function GET(_req: NextRequest) {
  ...
  if (lapErr) {
    console.warn('[admin/games/nordschleife] times error:', lapErr.message)
  }
  ...
  const { count: activeCount } = await supabase
    .from('nordschleife_presence')
    .select('id', { count: 'exact', head: true })
    .gte('last_seen', cutoff)

  // Total revenue from nordschleife-5-laps shop product (count of paid orders × €2)
  const { data: paidOrders, count: paidOrderCount } = await supabase
    ...
    .eq('status', 'paid')
    .eq('shop_products.slug', 'nordschleife-5-laps')
...
export async function DELETE(req: NextRequest) {
  ...
  const supabase = createServiceClient()
  const { error } = await supabase.from('nordschleife_times').delete().eq('id', id)
