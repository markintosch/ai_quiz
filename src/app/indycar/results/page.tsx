...
import SharePanel from '@/components/indycar/SharePanel'
...
function ResultsInner() {
  ...
  useEffect(() => {
    ...
    if (!d) { router.replace('/indycar'); return }
    try {
      const payload = JSON.parse(atob(d)) as LapPayload
      setLap(payload)
    } catch {
      router.replace('/indycar')
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/indycar/submit', {
        method: 'POST',
    ...
    <div style={{ minHeight: '100vh', background: BG, fontFamily: 'Inter, system-ui, sans-serif', color: WHITE }}>
      ...
      <nav style={{ borderBottom: `1px solid ${BORDER}`, padding: '0 24px', background: DARK }}>
        <div style={{ maxWidth: 720, margin: '0 auto', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Link href="/indycar" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
            <div style={{ width: 24, height: 24, borderRadius: 6, background: `linear-gradient(135deg, ${DEEP}, ${GREEN})`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: 12, fontWeight: 900, color: WHITE }}>I</span>
            </div>
            <span style={{ fontSize: 13, fontWeight: 800, color: WHITE }}>IndyCar</span>
          </Link>
        ...
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, overflow: 'hidden', marginBottom: 28 }}>
          ...
          <div style={{ padding: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: 1 }}>
              {[
                { label: t('sector_1'), name: 'Front Straight',     ms: s1, diff: t('difficulty_easy') },
                { label: t('sector_2'), name: 'Turn 1',      ms: s2, diff: t('difficulty_medium') },
                { label: t('sector_3'), name: 'Yard of Bricks', ms: s3, diff: t('difficulty_hard') },
              ].map((s, i) => (
...
export default function IndycarResultsPage() {
  return (
    <Suspense fallback={
      <div style={{ minHeight: '100vh', background: BG, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
