...
import { PAID_BUNDLE_ATTEMPTS } from '@/products/indycar/data'
...
function ClaimInner() {
  ...
  useEffect(() => {
    ...
    setState('claiming')
    fetch('/api/indycar/credits/claim', {
      method: 'POST',
        ...
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 18, padding: '36px 28px', textAlign: 'center' }}>
          ...
          {state === 'ok' && (
            <>
              ...
              <Link href="/indycar" style={{ fontSize: 13, color: MUTED, textDecoration: 'underline' }}>
                {t('claim_back')}
              </Link>
            </>
          )}
...
export default function IndycarClaimPage() {
  return (
    <Suspense fallback={
