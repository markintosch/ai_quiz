...
export default function IndycarPlayPage() {
  const router = useRouter()
  const { t, locale } = useIndycarLocale()
    ...
    fetch('/api/indycar/credits')
      ...
      .then(d => {
        ...
        if (freeAvailable > 0) {
          ...
          // burn a paid attempt
          fetch('/api/indycar/credits', { method: 'POST' })
            .then(r => r.json())
            .then(d => {
  ...
  const handleAnswer = useCallback((answer: string | null, timedOut = false) => {
    ...
    setTimeout(() => {
      ...
      if (qIndex + 1 >= questions.length) {
        ...
        router.push(`/indycar/results?d=${encoded}`)
      } else {
        setQIndex(qi => qi + 1)
        setPhase('racing')
      }
            ...
            {[1, 2, 3].map(s => {
              ...
              return (
                <div key={s} style={{
                  ...
                  background: isActive ? GREEN : isDone ? '#2A1A22' : CARD,
                  border: `1px solid ${isActive ? GREEN : isDone ? '#5A1A1F' : BORDER}`,
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '36px 24px 80px' }}>
            ...
            <button
              ...
              onMouseEnter={e => {
                if (phase === 'racing') {
                  ...
                  ;(e.currentTarget as HTMLButtonElement).style.background = '#2E1A1F'
                }
              }}
              onMouseLeave={e => {
        ...
        <div style={{ marginTop: 32, display: 'flex', gap: 2 }}>
          {questions.map((_, i) => {
            ...
            const color   = active ? GREEN : done ? (correct ? GREEN : '#7A0F1F') : BORDER
            return (
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6, fontSize: 10, color: MUTED }}>
          <span>S1 Front Straight</span>
          <span>S2 Turn 1</span>
          <span>S3 Yard of Bricks</span>
        </div>
      </div>
    </div>
  )
