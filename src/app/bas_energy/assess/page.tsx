...
function AssessHeader({ subTitle, backHref, backLabel }: { subTitle: string; backHref: string; backLabel: string }) {
  return (
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', width: '100%', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Link href="/bas_energy" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: '#64748B', lineHeight: 1.2, margin: 0 }}>{subTitle}</p>
          </div>
        </Link>
        <Link href={backHref} style={{ fontSize: 13, color: '#64748B', textDecoration: 'none', fontWeight: 500 }}>{backLabel}</Link>
      </div>
    </nav>
  )
}

function BasEnergyAssessInner() {
  ...
  const goToResults = () => {
    ...
    DIMENSIONS.forEach(dim => {
      ...
      dimScores[dim.id] = vals.length ? (vals as number[]).reduce((a, b) => a + b, 0) / vals.length : 1
    })
    const encoded = encodeURIComponent(btoa(JSON.stringify(dimScores)))
    router.push(`/bas_energy/results?d=${encoded}&role=${roleId}&lang=${lang}`)
  }
      ...
      <div style={{ minHeight: '100vh', background: '#F7FAFC', fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 60px)', padding: '48px 24px' }}>
          <div style={{ width: '100%', maxWidth: 540 }}>
            <span style={{ display: 'inline-block', fontSize: 11, fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: BRAND, border: `1px solid rgba(15,123,85,0.3)`, padding: '3px 12px', borderRadius: 4, marginBottom: 20 }}>{t.stepBadge}</span>
            <h1 style={{ fontSize: 28, fontWeight: 900, color: '#0F172A', marginBottom: 8, lineHeight: 1.2 }}>{t.selectTitle}</h1>
            <p style={{ fontSize: 15, color: '#64748B', lineHeight: 1.65, marginBottom: 32 }}>{t.selectBody}</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 28 }}>
              {ROLES.map(r => (
                <button key={r.id} onClick={() => setRoleId(r.id)} style={{ textAlign: 'left', padding: '16px 18px', borderRadius: 10, border: `2px solid ${roleId === r.id ? BRAND : '#E2E8F0'}`, background: roleId === r.id ? BRAND_LIGHT : '#fff', cursor: 'pointer', transition: 'all 0.15s' }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#0F172A', margin: '0 0 4px' }}>{r.label}</p>
                  <p style={{ fontSize: 12, color: '#64748B', lineHeight: 1.5, margin: 0 }}>{r.description}</p>
                </button>
              ))}
            </div>
            <button disabled={!roleId} onClick={() => setStep('scan')} style={{ width: '100%', padding: '14px 24px', borderRadius: 8, background: roleId ? BRAND : '#E2E8F0', color: roleId ? '#fff' : '#64748B', fontWeight: 700, fontSize: 15, border: 'none', cursor: roleId ? 'pointer' : 'not-allowed', transition: 'all 0.15s' }}>
              {t.selectCta}
            </button>
          </div>
        </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F7FAFC', fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" }}>
      ...
      <div style={{ maxWidth: 780, margin: '0 auto', padding: '32px 24px 72px' }}>
        <p style={{ fontSize: 12, color: '#64748B', textAlign: 'right', marginBottom: 12 }}>{t.progressLabel(totalAnswered, QUESTIONS.length)}</p>
        <div style={{ display: 'flex', gap: 6, marginBottom: 32, overflowX: 'auto', paddingBottom: 4 }}>
          {DIMENSIONS.map((dim, i) => {
            ...
            return (
              <button key={dim.id} onClick={() => setActiveDimension(i)} style={{ flexShrink: 0, padding: '6px 14px', borderRadius: 100, fontSize: 12, fontWeight: 700, border: `1.5px solid ${isActive ? BRAND : done ? 'rgba(15,123,85,0.35)' : '#E2E8F0'}`, background: isActive ? BRAND : done ? BRAND_LIGHT : '#fff', color: isActive ? '#fff' : done ? BRAND : '#64748B', cursor: 'pointer', transition: 'all 0.15s', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <span style={{ display: 'inline-block', fontSize: 11, fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: BRAND, border: `1px solid rgba(15,123,85,0.3)`, padding: '3px 12px', borderRadius: 4, marginBottom: 12 }}>
            {t.dimOf(activeDimension + 1, DIMENSIONS.length)}
          </span>
          <h2 style={{ fontSize: 22, fontWeight: 900, color: '#0F172A', marginBottom: 6, letterSpacing: '-0.01em' }}>{dimension.icon} {dimension.name}</h2>
          <p style={{ fontSize: 14, color: '#64748B', lineHeight: 1.65, maxWidth: 600 }}>{dimension.description}</p>
        </div>
          ...
          {dimQs.map(q => {
            ...
            return (
              <div key={q.id} style={{ background: '#fff', borderRadius: 12, padding: '24px 22px', border: '1px solid #E2E8F0' }}>
                <p style={{ fontSize: 15, fontWeight: 500, color: '#0F172A', marginBottom: 18, lineHeight: 1.65 }}>{q.text}</p>
                <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                  {([1, 2, 3, 4] as const).map(v => (
                    <button key={v} onClick={() => setAnswers(prev => ({ ...prev, [q.id]: v }))} style={{ flex: 1, padding: '11px 0', borderRadius: 8, fontWeight: 700, fontSize: 16, border: `2px solid ${selected === v ? BRAND : '#E2E8F0'}`, background: selected === v ? BRAND : '#F7FAFC', color: selected === v ? '#fff' : '#CBD5E0', cursor: 'pointer', transition: 'all 0.15s' }}>
                      {v}
                    </button>
                  ))}
                </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 28 }}>
          {activeDimension > 0 && (
            <button onClick={() => setActiveDimension(a => a - 1)} style={{ flex: 1, padding: '13px 24px', borderRadius: 8, border: '1.5px solid #E2E8F0', background: '#fff', fontWeight: 600, fontSize: 14, color: '#0F172A', cursor: 'pointer' }}>
              {t.prevBtn}
            </button>
          )}
          {activeDimension < DIMENSIONS.length - 1 ? (
            <button disabled={!dimAnswered} onClick={() => setActiveDimension(a => a + 1)} style={{ flex: 1, padding: '13px 24px', borderRadius: 8, background: 'transparent', color: dimAnswered ? BRAND : '#64748B', border: dimAnswered ? `2px solid ${BRAND}` : '2px solid #E2E8F0', fontWeight: 700, fontSize: 14, cursor: dimAnswered ? 'pointer' : 'not-allowed', transition: 'all 0.15s' }}>
              {t.nextBtn}
            </button>
          ) : (
            <button disabled={totalAnswered < QUESTIONS.length} onClick={goToResults} style={{ flex: 1, padding: '13px 24px', borderRadius: 8, background: totalAnswered >= QUESTIONS.length ? BRAND : '#E2E8F0', color: totalAnswered >= QUESTIONS.length ? '#fff' : '#64748B', border: 'none', fontWeight: 700, fontSize: 14, cursor: totalAnswered >= QUESTIONS.length ? 'pointer' : 'not-allowed', transition: 'all 0.15s' }}>
...
export default function BasEnergyAssessPage() {
  return (
